...
static void
Hide(Widget w, XEvent *ev, String *parms, Cardinal *np) {
...
main(int argc, char *argv[]) {
  ...
  // Clipboard Manager Specification https://freedesktop.org/wiki/ClipboardManager/
  ...
  (void) XtCreateManagedWidget("hide", Command, parent, NULL, ZERO);
  ...
  if (userOptions.wrap) {
    ...
    n++;
    XtSetArg(args[n], XtNscrollHorizontal, False);
    n++;
  }
  ...
  /* find /usr/share/icons -name \*.svg | grep status/16/ */
  char *task_active_icon = "task-past-due";
  char *task_attention_icon = "task-due";
  ...
  /* PASSIVE hides the icon, ACTIVE and ATTENTION states show different icons */
  app_indicator_set_status(ci, APP_INDICATOR_STATUS_ACTIVE);
  app_indicator_set_attention_icon_full(ci, task_attention_icon, "ICON_DESC");
